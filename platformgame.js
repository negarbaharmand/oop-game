  ...
  checkPlatformCollisions(platforms) {
    platforms.forEach((platform) => {
      if (this.collidesWith(platform)) {
        ...
        if (
          this.velocityY > 0 &&
          this.y + this.height - this.velocityY <= platform.y + 5
        ) {
  ...
  checkEnemyCollisions(enemies) {
    ...
    enemies.forEach((enemy) => {
      if (this.collidesWith(enemy)) {
        ...
        if (
          this.velocityY > 0 &&
          this.y + this.height - this.velocityY <= enemy.y + 10
        ) {
          enemy.defeat();
...
class Game {
  ...
  render() {
    ...
    this.collectibles.forEach((collectible) =>
      collectible.render(this.ctx, this.cameraX)
    );
  ...
  reset() {
    ...
    // Wait a tiny bit to ensure stop completes
    setTimeout(() => {
      this.score = 0;
      this.level = 1;
      this.initLevel();
...
function createGame() {
  // Destroy old game if it exists
  if (game) {
    console.log("🗑️ Destroying old game instance");
    game.stop();
    game = null;
  }
